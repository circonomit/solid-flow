...
export function NodeWrapper<NodeType extends Node>(
	...
	const initialNodeComponent = () =>
		p.nodeTypes?.[initialNodeType()] || builtinNodeTypes[initialNodeType()];
	...
	const isDraggable = () =>
		!!(
			nodeData.node()?.draggable ||
			(p.nodesDraggable && typeof nodeData.node()?.draggable === "undefined")
		);
	const isSelectable = () =>
		!!(
			nodeData.node()?.selectable ||
			(p.elementsSelectable &&
				typeof nodeData.node()?.selectable === "undefined")
		);
	const isConnectable = () =>
		!!(
			nodeData.node()?.connectable ||
			(p.nodesConnectable &&
				typeof nodeData.node()?.connectable === "undefined")
		);
	const isFocusable = () =>
		!!(
			nodeData.node()?.focusable ||
			(p.nodesFocusable && typeof nodeData.node()?.focusable === "undefined")
		);
	...
	const onSelectNodeHandler = (event: MouseEvent) => {
		if (
			isSelectable() &&
			(!store.selectNodesOnDrag.get() ||
				!isDraggable() ||
				store.nodeDragThreshold.get() > 0)
		) {
			/*
	...
	return (
		<div
			...
			aria-describedby={
				p.disableKeyboardA11y ? undefined : `${ARIA_NODE_DESC_KEY}-${p.rfId}`
			}
