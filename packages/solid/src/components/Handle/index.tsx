...
function HandleComponent(_p: HandleProps) {
	...
	const onPointerDown = (event: MouseEvent | TouchEvent) => {
		...
		if (
			...
			XYHandle.onPointerDown(event, {
				...
				isValidConnection:
					p.isValidConnection || currentStore.isValidConnection,
	...
	const onClick = (event: MouseEvent) => {
		...
		if (
			!currentNodeId ||
			(!connectionClickStartHandle && !p.isConnectableStart)
		) {
			return;
		}
		...
		const connectionClone = structuredClone(connectionState.get()) as Optional<
			ConnectionState,
			"inProgress"
		>;
		...
		<div
			...
			class={cc([
				...
				p.class,
				{
					...
					connectionindicator:
						p.isConnectable &&
						(!connectingData.connectionInProcess() ||
							connectingData.isPossibleEndHandle()) &&
						(connectingData.connectionInProcess() ||
						connectingData.clickConnectionInProcess()
							? p.isConnectableEnd
							: p.isConnectableStart),
				},
			])}
