...
export function EdgeWrapper<EdgeType extends Edge = Edge>(
	...
	const edge = () =>
		defaultEdgeOptions
			? { ...defaultEdgeOptions, ...storeEdge() }
			: storeEdge();

	const initialEdgeType = () => edge().type || "default";
	const InitialEdgeComp = () =>
		p.edgeTypes?.[initialEdgeType()] || builtinEdgeTypes[initialEdgeType()];
	...
	const isFocusable = () =>
		!!(
			edge().focusable ||
			(p.edgesFocusable && typeof edge().focusable === "undefined")
		);
	const isReconnectable = () =>
		typeof p.onReconnect !== "undefined" &&
		(edge().reconnectable ||
			(p.edgesReconnectable && typeof edge().reconnectable === "undefined"));
	const isSelectable = () =>
		!!(
			edge().selectable ||
			(p.elementsSelectable && typeof edge().selectable === "undefined")
		);
	...
	const markerStartUrl = () =>
		edge().markerStart
			? `url('#${getMarkerId(edge().markerStart, p.rfId)}')`
			: undefined;

	const markerEndUrl = () =>
		edge().markerEnd
			? `url('#${getMarkerId(edge().markerEnd, p.rfId)}')`
			: undefined;
		...
		<Show when={shouldShow()}>
			<svg style={{ "z-index": zIndex() }}>
				<g
					...
					aria-describedby={
						isFocusable() ? `${ARIA_EDGE_DESC_KEY}-${p.rfId}` : undefined
					}
