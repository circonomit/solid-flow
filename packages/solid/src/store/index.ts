...
const createStore = ({
	...
	const updateNodePositions: DefaultActions["updateNodePositions"] = (
		nodeDragItems,
		dragging = false,
	) => {
		return batch(() => {
	...
	const triggerNodeChanges: DefaultActions["triggerNodeChanges"] = (
		changes,
	) => {
		return batch(() => {
	...
	const triggerEdgeChanges: DefaultActions["triggerEdgeChanges"] = (
		changes,
	) => {
		return batch(() => {
	...
	const addSelectedNodes: DefaultActions["addSelectedNodes"] = (
		selectedNodeIds,
	) => {
		return batch(() => {
			...
			if (multiSelectionActive.get()) {
				const nodeChanges = selectedNodeIds.map((nodeId) =>
	...
	const addSelectedEdges: DefaultActions["addSelectedEdges"] = (
		selectedEdgeIds,
	) => {
		batch(() => {
			const { multiSelectionActive, edgeLookup, nodeLookup } = store;
	...
	const unselectNodesAndEdges = ({
		...
		batch(() => {
			...
			const edgeChanges = edgesToUnselect.map((edge) =>
	...
	const setTranslateExtent: DefaultActions["setTranslateExtent"] = (
		translateExtent,
	) => {
		batch(() => {
			const { panZoom } = store;
