...
import type {
	...
	MiniMapNodeProps,
	MiniMapNodes as MiniMapNodesProps,
} from "./types";

declare const window: any;

const selectorNodeIds = (s: SolidFlowState) => () =>
	s.nodes.get().map((node) => node.id);
const getAttrFunction = <NodeType extends Node>(
	func: any,
): GetMiniMapNodeAttribute<NodeType> =>
...
function NodeComponentWrapperInner<NodeType extends Node>(p: {
	...
	return (
		<Show
			when={
				storeData.node() &&
				!storeData.node()!.hidden &&
				nodeHasDimensions(storeData.node()!)
					? storeData.node()
					: false
			}
		>
...
const NodeComponentWrapper =
	NodeComponentWrapperInner as typeof NodeComponentWrapperInner;
