...
import type { ResizeControlLineProps, ResizeControlProps } from "./types";
...
function ResizeControl(_p: ResizeControlProps) {
	...
	const defaultPosition = () =>
		p.variant === ResizeControlVariant.Line ? "right" : "bottom-right";
			...
			resizer.current = XYResizer({
				...
				onChange: (
					...
					if (change.width !== undefined && change.height !== undefined) {
						const setAttributes = !p.resizeDirection
							? true
							: p.resizeDirection === "horizontal"
								? "width"
								: "height";
	...
	const colorStyleProp = () =>
		p.variant === ResizeControlVariant.Line ? "borderColor" : "backgroundColor";
	const controlStyle = (): JSX.CSSProperties | undefined =>
		p.color
			? ({ ...p.style, [colorStyleProp()]: p.color } as JSX.CSSProperties)
			: p.style;
