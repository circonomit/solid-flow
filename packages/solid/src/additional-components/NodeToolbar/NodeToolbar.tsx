...
import type { InternalNode, SolidFlowState } from "../../types";
...
export function NodeToolbar(_p: NodeToolbarProps) {
	...
	// }: NodeToolbarProps) {
	const p = mergeProps(
		{ position: Position.Top, offset: 10, align: "center" as Align },
		_p,
	);
	...
	const nodesSelector =
		...
		() => {
			const nodeIds = Array.isArray(p.nodeId)
				? p.nodeId
				: [p.nodeId || getContextNodeId() || ""];
	...
	const isActive = () =>
		typeof p.isVisible === "boolean"
			...
			: nodes.length === 1 &&
				nodes()[0].selected &&
				storeData.selectedNodesCount === 1;
	...
	const nodeRect = (): Rect =>
		getNodesBounds(nodes(), { nodeOrigin: storeData.nodeOrigin });
	const zIndex = (): number =>
		Math.max(...nodes().map((node) => node.internals.z + 1));
