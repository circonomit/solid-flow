...
import {
	defaultNodeOrigin,
	defaultViewport as initViewport,
} from "./init-values";
...
type UsedProps<N extends Node, E extends Edge> = SolidFlowProps<N, E> & {
...
function SolidFlow<NodeType extends Node = Node, EdgeType extends Edge = Edge>(
	_p: UsedProps<NodeType, EdgeType>,
) {
	//   {
	//     nodes,
	//     edges,
	...
	const p = mergeProps(
		{
			...
			noPanClassName: "nopan",
			disableKeyboardA11y: false,
		} satisfies Partial<SolidFlowProps<NodeType, EdgeType>>,
		_p,
	);
	...
	const wrapperOnScroll = (e: Event) => {
		...
		if (typeof p.onScroll === "function") {
			p.onScroll(
				e as Event & { currentTarget: HTMLDivElement; target: Element },
			);
		}
	};
	...
	return (
		<div
			...
			style={{
				...(typeof p.style === "object" ? p.style : null),
				...wrapperStyle,
			}}
			ref={p.ref}
		...
		>
			...
			>
				...
				{p.children}
				<Attribution
					proOptions={p.proOptions}
					position={p.attributionPosition}
				/>
				<A11yDescriptions
					rfId={rfId()}
					disableKeyboardA11y={p.disableKeyboardA11y}
