...
import type { SolidFlowProps, SolidFlowState } from "../../types";
...
export function Pane(_p: ParentProps<PaneProps>) {
	...
	const hasActiveSelection = () =>
		...
		(p.isSelecting || storeData.userSelectionActive());
	...
	const onWheel = p.onPaneScroll
		? (event: WheelEvent) => p.onPaneScroll!(event)
		: undefined;
	...
	const onPointerDown = (event: PointerEvent): void => {
		...
		(event.target as Partial<Element> | null)?.setPointerCapture?.(
			event.pointerId,
		);
	...
	const onPointerUp = (event: PointerEvent) => {
		...
		(event.target as Partial<Element>)?.releasePointerCapture?.(
			event.pointerId,
		);
		const userSelectionRect = store.userSelectionRect.get();
	...
	const draggable = () =>
		p.panOnDrag === true ||
		(Array.isArray(p.panOnDrag) && p.panOnDrag.includes(0));
		...
		<div
			class={cc([
				"solid-flow__pane",
				{
					...
					selection: p.isSelecting,
				},
			])}
			onClick={
				hasActiveSelection() ? undefined : wrapHandler(onClick, container)
			}
