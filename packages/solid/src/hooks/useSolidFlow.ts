...
>(): ReactFlowInstance<NodeType, EdgeType> {
	...
	const generalHelper = createMemo<GeneralHelpers<NodeType, EdgeType>>(() => {
		const getInternalNode: GeneralHelpers<
			NodeType,
			EdgeType
		>["getInternalNode"] = (id) =>
			store.nodeLookup.get(id) as InternalNode<NodeType>;

		const setNodes: GeneralHelpers<NodeType, EdgeType>["setNodes"] = (
			payload,
		) => {
			batchContext.nodeQueue.push(payload as NodeType[]);
		};

		const setEdges: GeneralHelpers<NodeType, EdgeType>["setEdges"] = (
			payload,
		) => {
		...
		const getNodeRect = (node: NodeType | { id: string }): Rect | null => {
			...
			const nodeToUse = isNode<NodeType>(node)
				? node
		...
		return {
			...
			deleteElements: async ({
				...
				if (hasMatchingEdges) {
					const edgeChanges: EdgeRemoveChange[] = matchingEdges.map(
				...
				if (hasMatchingNodes) {
					const nodeChanges: NodeRemoveChange[] = matchingNodes.map(
						elementToRemoveChange,
					);
			...
			fitView: async (options: FitViewOptions<NodeType> | undefined) => {
				...
				const fitViewResolver =
					store.fitViewResolver.get() ?? withResolvers<boolean>();
